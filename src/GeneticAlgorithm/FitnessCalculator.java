...
public class FitnessCalculator {
    ...
    // The idea behind this fitness function is fairly simple, it's based on the
    // expected frequency of letter combinations in english.
    // This is only one possible implementation of a fitness function. Feel free to
    // do some research on your own, and test different approaches
    // The general idea in this function is to compare the frequencies which letter
    // combinations occur in the de-shredded document, to the frequencies you would
    // expect to observe in english
    // Smaller values indicate better solutions
    public static double fitness(char[][] shreds, int[] perm) {
        // The expected frequency of different character combinations in english
        // language text according to
        ...
        // Yes the above is gross, but it works. No I didn't type it all out, I wrote a
        // script to parse a file containing expected frequencies
        ...
        for (int i = 0; i < shreds[0].length; i++) {
            for (int j = 1; j < shreds.length; j++) {
                ...
                // When the current combination consists of letters, increment the count for
        ...
        for (int line = 0; line < shreds[0].length; line++) {
            for (int i = 0; i < 26; i++) {
                for (int j = 0; j < 26; j++) {
                    // If we know the expected frequency of the combination take the absolute
                    // difference between the observed frequency
                    // and the expect frequency
                    // In general score will be smaller when the de-shredded text looks more like
                    // English
                    double expected = expectedFrequencies[i][j];

                    double calculatedScore = abs((((float) charCounts[line][i][j]) / totalChars[line]) - expected)
                            * (1.0 - expected);
                    // If you're wondering about the (1.0-expected) above. expected is a frequency
                    // between 0.0 and 1.0, so (1.0-expected) will be larger when expected is
                    // smaller. Basically this means that our fitness function will place extra
                    // emphasis on less common letter combinations.
                    // For an example of why this might be helpful: the letter combination qz almost
                    // never occurs in english, so 1.0-expected is relatively large for qz. As a
                    // result our fitness function will essentially treat it as extra bad when qz
                    // appears too often
        ...
        for (int i = 0; i < score.length; i++)
            total += pow(score[i], 2);
        // Using pow(x,2) here essentially makes the fitness function focus more on
        // lines with a bad score, since squaring will exaggerate the differences
        // between scores
        // This should help encourage the GA to choose solutions which do well for all
        // lines, instead of solutions that do REALLY well for only a few lines
    ...
    /**
     * This method will print out a document in an easy to ready format. Note: doc
     * MUST BE A SQUARE ARRAY. ALL ROWS MUST BE THE SAME LENGTH
     */
